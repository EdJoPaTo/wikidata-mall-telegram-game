...
test('numberArrayFilterIsFinite Infinity gone', numberArrayFilterIsFiniteMacro, [], [Number.POSITIVE_INFINITY])
test('numberArrayFilterIsFinite 0 still there', numberArrayFilterIsFiniteMacro, [0], [0])
test('numberArrayFilterIsFinite big example', numberArrayFilterIsFiniteMacro, [0, 2, 4], [0, Number.POSITIVE_INFINITY, null, 2, Number.NaN, undefined, 4])

const calcQuickStatsMacro = createInputOutputDeepEqualMacro(calcQuickStats)

test('calcQuickStats empty input', calcQuickStatsMacro, {amount: 0, sum: 0, avg: Number.NaN, min: Number.POSITIVE_INFINITY, max: Number.NEGATIVE_INFINITY}, [])
